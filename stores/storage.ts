...
export const useStorageStore = defineStore('storage', {
  ...
  actions: {
    ...
    async loadStorageSubCategory(params: { slug: string | string[], search?: string | number }): Promise<void> {
      await axios.get(`api/v1/store/subcategory/${params.slug}?search=${params.search}`, {
        headers: {
          Authorization: `Bearer ${nuxtStorage.localStorage.getData('token')}`
        }
      }).then(res => {
    ...
    async loadStorageCategorySearch(params: {query: string | number}): Promise<void> {
      await axios.get(`api/v1/store/category/category-list?name=${params.query}`, {
        headers: {
          Authorization: `Bearer ${nuxtStorage.localStorage.getData('token')}`
        }
      }).then(res => {
    ...
    async loadAllListSearch(params: { search?: any }): Promise<void> {
      await axios.get(`api/v1/store/item/item-list?search=${params.search}`, {
        headers: {
          Authorization: `Bearer ${nuxtStorage.localStorage.getData('token')}`
        }
      }).then(res => {
    ...
    async loadItem(params: { itemSlug: string | string[] }): Promise<void> {
      await axios.get(`api/v1/store/item/${params.itemSlug}`, {
        headers: {
          Authorization: `Bearer ${nuxtStorage.localStorage.getData('token')}`
        }
      }).then(res => {
