...
function messages() {
  let mes = new WebSocket(`wss://api-buildwithus.ai-softdev.com/ws/chat/${route.params.id}?token=${localStorage.getItem('token')}`)
  mes.onmessage = (event) => {
    ...
    if (chat.userChat?.id === res.chat_id && mes.readyState === WebSocket.OPEN) {
      ...
      console.log(chat.userChat.messages[0])
    }
...
<template>
  <div class="h-full  w-full">
    <div
      class="w-full flex items-center border-l max-h-[8%] max-md:pt-3 max-md:px-14 h-[100px] px-10 border-black dark:border-opacity-20 border-opacity-10 bg-gray-200 dark:bg-gray-700">
      ...
      <div v-else class="flex items-center gap-x-2 cursor-pointer" @click="showChatInfo = true">
    ...
    <TheModal :type="'resizeInfoWindow'" v-if="showChatInfo && chat.userChat?.is_group"
              @showModal="showChatInfo = false">
      <div class="p-4 flex justify-between items-center">
        <p class="tracking-widest">{{ $t('Информация о группе') }}</p>
        <div class="cursor-pointer" @click="showChatInfo = false">
      ...
      <div
        ...
        <p class="uppercase text-sm ml-4">{{ chat.get_user_chat.users.length + ' ' }}<span>{{ $t('участников') }}</span></p>
      </div>
