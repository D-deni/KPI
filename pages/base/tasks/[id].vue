...
<template>
  <TheContentBlock class="w-11/12 mx-auto max-md:w-full relative">
    ...
    <div class="flex mx-6 max-sm:mx-0 gap-x-10 max-lg:flex-col">
      <div class="my-10 w-10/12 max-lg:w-full">
        ...
        <div>
          ...
          <div class="w-full flex flex-wrap gap-x-4 gap-y-2 items-center mx-auto border p-10 my-6 border-gray-400 rounded-lg" >
            <a v-for="files in currentTask.task.author_files_urls"
              ...
              <div>
                ...
                <div
                    v-else-if="files.file_name?.slice(0 - 3) === 'doc' || files.file_name?.slice(0 - 4) === 'docs'">
                  ...
                  <svg width="60px" height="60px" viewBox="-4 0 64 64" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <g fill-rule="evenodd" clip-rule="evenodd">
                      <path d="M56.008 20.316v1h-12.799s-6.312-1.26-6.129-6.708c0 0 .208 5.708 6.004 5.708h12.924z"
                            fill="#F7BC04"/>
                      <path d="M37.106-.036v14.561c0 1.656 1.104 5.792 6.104 5.792h12.799l-18.903-20.353z" opacity=".5"
                            fill="#ffffff"/>
                    </g>
                    <path
            ...
            <div
              ...
              <div class="text-center mt-10">
                ...
                <p class="mt-2 text-gray-400">{{ $t('Отправлять файлы может только') }} <span
                  class="font-bold dark:text-gray-300 text-black tracking-widest">{{ currentTask.task.main_performer?.first_name + ' ' + currentTask.task.main_performer?.last_name
                  }}</span>
                </p>
        ...
        <div class="mt-2">
          ...
          <NuxtLink :to="`/base/users/${currentTask.task.main_performer?.id}`"
            ...
            <div class=" max-md:w-full">
              <img class="w-[60px] h-[60px] rounded-full" :src="currentUser.get_server_domain + currentTask.task.main_performer?.photo_url" alt="">
            </div>
            <p>{{ currentTask.task.main_performer?.first_name + ' ' + currentTask.task.main_performer?.last_name }}</p>
          </NuxtLink>
        </div>
        <div>
          ...
          <div
            ...
            <div
              ...
              <NuxtLink :to="`/base/users/${user.id}`" class="w-11/12 flex items-center gap-x-4">
                <img class="w-[60px] h-[60px] rounded-full" :src="currentUser.get_server_domain + user?.photo_url" alt="">
    ...
    <div v-if="currentTask.task.status === 'watching' && currentTask.task.author_id === currentUser.user.id" class="flex flex-wrap gap-y-4 justify-center mb-10 gap-x-4">
    ...
    <div class="absolute max-md:fixed max-md:top-0 max-md:bottom-6 max-md:items-end stroke-white flex items-center top-14 right-6 gap-x-6">
    ...
    <TheModal v-if="showCancelError" class="!w-[200px]" @showModal="showCancelError = !showCancelError">
      ...
      <form @submit.prevent="currentTask.change_taskStatus({task_status:'make-rejected', id: currentTask.task.id, comment: updateForm.comment}); showCancelError = false"
            class="mt-10">
        <div>
          <p class="mb-4">{{$t('Опишите причину отклонения')}}</p>
          <textarea class="w-full resize-none h-[300px] p-4 border dark:border-none rounded-lg" v-model="updateForm.comment"></textarea>
        </div>
        <div class="flex justify-center gap-x-10">
    ...
    <TheModal v-if="showDeleteTask" @showModal="showDeleteTask = !showDeleteTask">
      ...
      <div class="my-6">
        ...
        <div class="flex gap-4 mt-2 justify-center">
          <TheButton type="danger" class="py-2 px-10 rounded-full" @click="showDeleteTask = false">{{$t('Отменить')}}</TheButton>
          <TheButton type="success" class="py-2 px-10 rounded-full" @click="currentTask.delete_task({id: currentTask.task.id}, $router); ">{{$t('Удалить')}}</TheButton>
        </div>
      </div>
    </TheModal>
  </TheContentBlock>
