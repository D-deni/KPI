...
<template>
  <div>
    <button @mouseenter="moveEffectCategory = true" @mouseleave="moveEffectCategory = false" class="flex items-center justify-between w-full relative" @click="isActiveCategory = !isActiveCategory">
      <div class="flex items-center gap-x-3 ">
        <div>
          <svg class="dark:stroke-white stroke-black" width="30px" height="30px" viewBox="0 0 24 24" fill="none"
               xmlns="http://www.w3.org/2000/svg">
            <path d="M3.17004 7.43994L12 12.5499L20.77 7.46991" stroke-width="1.2" stroke-linecap="round"
                  stroke-linejoin="round"/>
            <path d="M12 21.6099V12.5399" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/>
            <path
            ...
            <path d="M17 13.24V9.58002L7.51001 4.09998" stroke-width="1.2" stroke-linecap="round"
                  stroke-linejoin="round"/>
          </svg>
        </div>
        ...
        <Transition name="fade">
          <button v-if="moveEffectCategory" class="absolute right-5 rounded-lg" @click.stop="moveEffectCategoryWarn = true">
      ...
      <div :class="{'rotate-180' : isActiveCategory}" class="transition-all">
        <svg class="dark:fill-white fill-black" :class="{'!fill-semiCyan' : isActiveCategory}" width="20px" height="20px"
    ...
    <Transition name="slide-down" class="text-center">
      <TheModal type="resize" v-if="moveEffectCategoryWarn" @showModal="moveEffectCategoryWarn = false">
        <TheTextContent>{{$t('Удалить коробку?')}}</TheTextContent>
        <div class=" break-words tracking-wider my-4 text-sm">
          <p>{{$t('Если вы удалите коробку то всё содержимое данной коробки удалиться без возврата')}}</p>
          <p class="mt-4">{{$t('Убедитесь что коробка пустая, переместите нужные товары в другие категории')}}</p>
        </div>
        <div class="flex flex-wrap gap-x-10 justify-center">
          <TheButton type="chat" @click="moveEffectCategoryWarn = false">{{$t('Отмена')}}</TheButton>
          <TheButton type="chat" @click="storageStore.deleteStorageElem({deleteType: 'category', id: item.id})">{{$t('Удалить')}}</TheButton>
        </div>
      </TheModal>
    </Transition>
    <Transition name="slide-down">
      <div v-if="item.sub_categories && isActiveCategory">
        <StorageNavElemGoods class="mt-2" v-for="child in item.sub_categories" :item="child" :key="child.id"/>
      </div>
    </Transition>
  </div>
</template>
