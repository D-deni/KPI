...
<template>
  <div @contextmenu.prevent :id="item.id">
    <div class="flex items-end gap-x-2 p-1 rounded-lg "
         :class="{'max-md:justify-end' : item.sender.id === currentUser.user.id, 'w-7/12 max-md:w-full text-center ' : item.message_type !== 'default'}">
      <div v-if="item.message_type === 'default'"
           class="rounded-full max-md:hidden w-[30px]"
           :class="{'max-md:!block': chat.userChat.is_group, 'max-md:!hidden' : item.sender.id === currentUser.user.id}">
        <slot name="userImage"/>
      </div>
      <div :class="{
            'mx-auto py-1 px-4 dark:text-gray-300 text-gray-500 tracking-wider rounded-full dark:!bg-black dark:!bg-opacity-30' : item.message_type !== 'default',
            '!bg-transparent dark:!bg-transparent !p-0 !shadow-none' : item.message_type === 'change' && item.file,
            'flex flex-col py-1 px-2 gap-x-2 rounded-xl max-w-[40%] max-lg:max-w-[50%] max-md:max-w-[80%] relative before:absolute before:border-l-[5px] before:border-l-transparent before:border-r-[10px] before:border-b-[10px] before:border-r-transparent before:w-[3px] before:h-[3px] before:bottom-0' : item.message_type === 'default',
            'dark:bg-gray-600 bg-gray-200 dark:shadow-none shadow-md max-md:rounded-r-xl max-md:rounded-l-lg': item.sender.id !== currentUser.user.id,
        ...
        <p v-if="item.forwarded_from?.id && item.message_type === 'default'"
           class="text-semiCyan font-bold text-[12px]">
          <slot name="userForwarded"></slot>
        </p>
        <div class="break-words relative">
          <p class="text-[12px] text-blueSemiLight tracking-widest"
             v-if="chat.userChat.is_group && !item.forwarded_from?.id && item.sender.id !== currentUser.user.id && item.message_type === 'default'">
            <slot name="userSenderName"/>
          </p>
          <div class="" v-if="item.file && item.message_type === 'default'">
            <div class="flex gap-x-4 p-2 items-center cursor-pointer" v-if="item.file_name.substr(-4) !== '.png'"  @click="downloadWithAxios(item)">
              ...
              <div class="float-start break-all">
                <p>{{item.file_name?.length > 20 ? item.file_name.slice(0,15) + '...' + item.file_name.substr(-6) : item.file_name}}</p>
                <span class="text-sm dark:text-gray-500 text-gray-400 font-bold">{{item.file_size}} <span>{{ item.file_size > 1000 ? $t('КБ') : $t('Б')}}</span> </span>
              </div>
            </div>
            <div class="cursor-pointer max-w-[300px]" @dragstart.prevent v-if="item.file_name.substr(-4) === '.png'">
          ...
          <div class="text-sm  break-all"
            ...
            <div v-if="item.message_type === 'change' && item.file">
              <img class="rounded-full w-[80px] h-[80px] cursor-pointer"
                   :src="currentUser.get_server_domain + item.file" alt="">
            </div>
            <div v-if="item.message_type === 'default'" class="flex items-center justify-end gap-x-1 float-right ml-2"
              ...
              <p v-if="new Date(item.updated_at).toLocaleString() !== new Date(item.created_at).toLocaleString()"
                 class="text-[12px] tracking-wider font-light dark:text-gray-400 text-gray-400">{{ $t('изменено') }}</p>
