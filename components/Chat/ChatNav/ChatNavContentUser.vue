...
function message(chat_id: Number) {
  let socket = new WebSocket(`wss://buildwithus.ai-softdev.com/ws/chat/${chat_id}?token=${localStorage.getItem('token')}`)
...
<template>
  <NuxtLink :to="`/base/chat/${chat.id}`"
            ...
            @click="chatStore.activeChatNav = false; message(chat.id);  chatStore.showPinnedWindow = false; chatStore.messageUpdate = {}"
            ...
            :class="{'dark:hover:!bg-semiCyan ' : route.path === `/base/chat/${chat.id}`, 'hover:!bg-cyan' : route.path === `/base/chat/${chat.id}`}">
    <div class="flex gap-x-2">
      <TheChatAvatar :user="user" :chat="chat"/>
