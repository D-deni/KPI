<template>
  <div class="container z-50 relative shadow-lg dark:border-none"
    ...
    <div class="flex w-full items-center transition-all justify-between text-center bg-cyan "
         :class="{'px-3 py-[33px]' : !loadCurrentUser.activeNav, 'px-6 py-[25px]' : loadCurrentUser.activeNav}">
      ...
      <NavigationBlock>
        ...
        <template v-slot:navTitleIcon>
          ...
          <div v-else class="w-[40px] h-[40px]">
            <img class="w-full rounded-full" :src="loadCurrentUser.get_server_domain + loadCurrentUser.user.company_logo_url" alt="">
          </div>
        </template>
        <template v-slot:navContent>
